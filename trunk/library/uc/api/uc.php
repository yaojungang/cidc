...
class uc_note {
	...
	function synlogin($get, $post) {
		...
		//_setcookie('Zend_Auth', _authcode($uid."\t".$username, 'ENCODE'));
		setcookie('Zend_Auth',serialize(array('uid'=>$uid,'username'=>$username)),time()+60*60*24*30,'/');
    }

	function synlogin0($get, $post) {
		$uid = $get['uid'];
		$username = $get['username'];
		if(!API_SYNLOGIN) {
			return API_RETURN_FORBIDDEN;
	...
	function synlogout($get, $post) {
		...
		setcookie('Zend_Auth','',time()-60*60*24*3,'/');
	}
	function synlogout0($get, $post) {
		if(!API_SYNLOGOUT) {
			return API_RETURN_FORBIDDEN;
		}
